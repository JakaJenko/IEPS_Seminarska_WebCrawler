...
class PageBusinessController(AbstractDatabaseBusinessController):
    ...
    def Update(self, page_info):
        ...
        cur.execute("""UPDATE crawldb.page
                        ...
                        WHERE id = %s""",
                    (page_info.site_id, page_info.page_type_code, page_info.url, page_info.html_content, page_info.http_status, page_info.accessed_time, page_info.id))
        cur.close()

    Update(PageInfo(id=1, url="test"))