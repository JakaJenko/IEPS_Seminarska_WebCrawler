...
class Posting():
    def __init__(self, word, document, indexes=[]):
        ...
        self.frequency = str(len(indexes))
        self.indexes = indexes

def Insert_Posting(posting):
    cur = conn.cursor()
    cur.execute("""INSERT INTO ieps3.posting (word, documentName, frequency, indexes)
                VALUES (%s, %s, %s, %s)""",
                (posting.word, posting.document, posting.frequency, ','.join(str(i) for i in posting.indexes)))
...
def Insert_IndexWord(word):
    cur = conn.cursor()
    try:
        cur.execute("""INSERT INTO ieps3.indexWord (word)
                    VALUES (%s)""", [word])
    except:
        print("Word already in database: {}".format(word))
